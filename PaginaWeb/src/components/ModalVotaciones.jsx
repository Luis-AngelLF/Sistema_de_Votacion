...
export default function ModalVotaciones({ show, onClose, id_eleccion, cedula, idUsuario }) {
  ...
  const handleConfirmarVoto = async () => {
    console.log("Intentando votar - selected:", selected, "cedula:", cedula, "idUsuario:", idUsuario);
    ...
    try {
      // Enviar solo el ID del candidato seleccionado
      // El backend se encargará del cifrado y procesamiento
      const response = await fetch(`${API_BASE}/api/votar`, {
        ...
        body: JSON.stringify({
          ...
          id_candidato: selected, // Solo enviar el ID del candidato
        }),
      });
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
              {candidatos.map((candidato) => (
                ...
                >
                  {/* Foto del candidato */}
                  <div className="w-full h-40 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 rounded-lg flex items-center justify-center mb-3 border border-white/10 overflow-hidden">
                    {candidato.foto_url ? (
                      <img
                        src={candidato.foto_url}
                        alt={`Foto de ${candidato.nombre_completo}`}
                    ...
                    <div
                      className={`w-full h-full flex items-center justify-center ${candidato.foto_url ? 'hidden' : ''}`}
                    >
            ...
            <button
              ...
              `}
            >
              {votando ? "Registrando voto. NO REFRESQUE LA PÁGINA " : "Confirmar selección"}
            </button>
          </>
        )}
